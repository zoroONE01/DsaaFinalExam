...
/**
 * @brief Chuyển đổi tiêu chí tìm kiếm thành tiêu chí sắp xếp tương ứng
 */
int mapSearchToSortCriteria(int searchCriteria)
{
    ...
    {
    case SEARCH_BY_STUDENT_ID:
        return SORT_BY_STUDENT_ID; // 1 -> 1
    case SEARCH_BY_FIRST_NAME:
    case SEARCH_BY_LAST_NAME:
        return SORT_BY_NAME; // 2,3 -> 2
    case SEARCH_BY_SCORE:
        return SORT_BY_SCORE; // 5 -> 3
    default:
        return -1; // Không hỗ trợ binary search
    }
}

/**
 * @brief Kiểm tra xem có thể sử dụng binary search không
 */
...
SearchResult binarySearchArray(const ArrayStudentList &list, const char *keyword, int searchCriteria, int sortCriteria)
{
    ...
    else if (searchCriteria == SEARCH_BY_STUDENT_ID && sortCriteria == SORT_BY_STUDENT_ID)
    {
        // Binary search cho mã sinh viên với partial matching
        int keywordLen = strlen(keyword);

        // Tìm vị trí đầu tiên có thể khớp
        int left = 0, right = list.count - 1;
        int startIndex = -1;

        // Tìm vị trí bắt đầu của range có thể khớp
        while (left <= right)
        {
            int mid = (left + right) / 2;
            int cmp = strncmp(list.students[mid].studentID, keyword, keywordLen);
        ...
        if (startIndex != -1)
        {
            // Tìm tất cả sinh viên có mã bắt đầu bằng keyword
            int matchCount = 0;

            // Đếm số lượng match từ startIndex về sau
            for (int i = startIndex; i < list.count; i++)
            {
                if (containsIgnoreCase(list.students[i].studentID, keyword))
                {
                    matchCount++;
                }
                else if (strncmp(list.students[i].studentID, keyword, keywordLen) != 0)
                {
                    // Nếu không còn khớp prefix thì dừng
                    break;
                }
            }

            // Đếm thêm từ startIndex về trước (nếu có)
            ...
            if (matchCount > 0)
            {
                ...
                int resultIndex = 0;
                // Thu thập kết quả từ trước startIndex
                for (int i = startIndex - 1; i >= 0; i--)
                {
                    if (containsIgnoreCase(list.students[i].studentID, keyword))
                    {
                ...
                // Thu thập kết quả từ startIndex về sau
                for (int i = startIndex; i < list.count && resultIndex < matchCount; i++)
                {
                    if (containsIgnoreCase(list.students[i].studentID, keyword))
                    {
                        result.students[resultIndex++] = list.students[i];
        ...
        // Nếu không tìm thấy kết quả nào bằng binary search, fallback về sequential search
        if (result.count == 0)
        {
            result.isBinarySearch = false;
            stopTimer(timer);
    ...
    else if ((searchCriteria == SEARCH_BY_FIRST_NAME || searchCriteria == SEARCH_BY_LAST_NAME) && sortCriteria == SORT_BY_NAME)
    {
        // Binary search cho tên (có thể tìm nhiều kết quả)
        result.isBinarySearch = false; // Chuyển về sequential search vì tên có thể trùng và phức tạp
        stopTimer(timer);
        return sequentialSearch(const_cast<Student *>(list.students), list.count, keyword, searchCriteria);
    }
...
SearchResult searchDoublyLinkedList(NodeDLL *head, const char *keyword, int searchCriteria, bool isSorted, int sortCriteria)
{
    ...
    if (useBinarySearch && (searchCriteria == SEARCH_BY_SCORE || searchCriteria == SEARCH_BY_STUDENT_ID))
    {
        ...
        else if (searchCriteria == SEARCH_BY_STUDENT_ID)
        {
            // Binary search cho mã sinh viên với partial matching
            int keywordLen = strlen(keyword);
            int left = 0, right = totalNodes - 1;
            NodeDLL *foundNode = nullptr;

            // Tìm vị trí có thể khớp với prefix
            while (left <= right)
            {
                int mid = (left + right) / 2;
                ...
                int cmp = strncmp(midNode->info.studentID, keyword, keywordLen);
                if (cmp == 0)
                {
                    foundNode = midNode;
                    break; // Tìm thấy một node khớp prefix
                }
                else if (cmp < 0)
                {
                    left = mid + 1;
            ...
            {
                // Đếm tất cả node có mã sinh viên chứa keyword
                int matchCount = 0;
                NodeDLL *current = head;
                while (current != nullptr)
                {
                    if (containsIgnoreCase(current->info.studentID, keyword))
                    {
                        matchCount++;
                    }
                    current = current->next;
