...
void displaySearchResultsArrayWithOptions(const ArrayStudentList &list, int* indices, int count, 
                                         const char* keyword, int searchCriteria, bool showReversed, 
                                         double searchTimeMs);

// --- Tìm kiếm thông minh (tự động chọn binary/linear) ---
int* smartSearchArrayList(const ArrayStudentList &list, const char* keyword, int searchCriteria, 
                         int &count, bool isSorted, double &searchTimeMs);
